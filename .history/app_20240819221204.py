...
def merge_information(ocr_info, pypdf_info):
    ...
    def normalize_date(date_str):
        try:
            return parse(date_str, fuzzy=True).strftime("%Y-%m")
        except:
    ...
    def are_entries_similar(entry1, entry2, threshold=0.8):
        if isinstance(entry1, dict) and isinstance(entry2, dict):
            similarities = [string_similarity(str(entry1.get(k, '')), str(entry2.get(k, ''))) for k in set(entry1.keys()) | set(entry2.keys())]
            return sum(similarities) / len(similarities) >= threshold
        else:
            return string_similarity(str(entry1), str(entry2)) >= threshold
        ...
        if isinstance(entry1, dict) and isinstance(entry2, dict):
            ...
            for key in set(entry1.keys()) | set(entry2.keys()):
                if key in entry1 and key in entry2:
                    if isinstance(entry1[key], (str, int, float)) and isinstance(entry2[key], (str, int, float)):
                        merged[key] = max(str(entry1[key]), str(entry2[key]), key=len)
                    else:
                        merged[key] = merge_entries(entry1[key], entry2[key])
                elif key in entry1:
                    merged[key] = entry1[key]
                else:
                    merged[key] = entry2[key]
            return merged
        else:
            return max(str(entry1), str(entry2), key=len)

    def merge_lists(list1, list2):
        merged = []
        for item1 in list1:
            similar_item = next((item2 for item2 in list2 if are_entries_similar(item1, item2)), None)
            if similar_item:
                merged.append(merge_entries(item1, similar_item))
            else:
                merged.append(item1)
        
        for item2 in list2:
            if not any(are_entries_similar(item2, item) for item in merged):
                merged.append(item2)
        
        return merged

    def calculate_confidence(entry, ocr_entries, pypdf_entries):
        ocr_match = any(are_entries_similar(entry, ocr_entry) for ocr_entry in ocr_entries)
        pypdf_match = any(are_entries_similar(entry, pypdf_entry) for pypdf_entry in pypdf_entries)
        return 1.0 if ocr_match and pypdf_match else 0.5 if ocr_match or pypdf_match else 0.0

    merged_info = {}
    for key in set(ocr_info.keys()) | set(pypdf_info.keys()):
        if key in ocr_info and key in pypdf_info:
            if isinstance(ocr_info[key], list) and isinstance(pypdf_info[key], list):
                merged_list = merge_lists(ocr_info[key], pypdf_info[key])
                merged_info[key] = [
                    entry for entry in merged_list
                    if calculate_confidence(entry, ocr_info[key], pypdf_info[key]) > 0.5
                ]
            elif isinstance(ocr_info[key], dict) and isinstance(pypdf_info[key], dict):
                merged_info[key] = merge_entries(ocr_info[key], pypdf_info[key])
            else:
                merged_info[key] = ocr_info[key] if len(str(ocr_info[key])) >= len(str(pypdf_info[key])) else pypdf_info[key]
        elif key in ocr_info:
            merged_info[key] = ocr_info[key]
        else:
            merged_info[key] = pypdf_info[key]
