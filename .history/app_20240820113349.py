...
def merge_information(ocr_info, pypdf_info):
    def merge_lists(item1, item2):
        if isinstance(item1, list) and isinstance(item2, list):
            if all(isinstance(i, dict) for i in item1 + item2):
                # For lists of dictionaries, we can't use set()
                merged = item1 + item2
                ...
                # For lists of hashable items
                return list(set(item1 + item2))
            ...
            for key, value in item2.items():
                if key in merged:
                    if isinstance(merged[key], list) and isinstance(value, list):
                        ...
                        merged[key] = value if value else merged[key]
                else:
                    merged[key] = value
            return merged
        else:
            # If types don't match, return the non-empty item or item1 if both are non-empty
            return item1 if item1 else item2

    def merge_education(ocr_edu, pypdf_edu):
        return merge_lists(ocr_edu, pypdf_edu)
    ...
    for key in all_keys:
        if key not in ocr_info:
            ...
            if key in ['education', 'work_experience', 'skills']:
