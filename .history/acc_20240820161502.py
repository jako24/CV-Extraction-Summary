...
# SequenceMatcher will be good for the following fields -> soft skills, technical skills, branch, degree
# The other fields has to be treated more strictly. if all sequence correct - 100% if not 0% [name, email, phone, grad_year, start_date, end_date, ]
...
def sequence_match_accuracy(extracted_value, correct_value):
    ...
    if correct_value is None:
        correct_value = ""
    print(f"Comparing: Extracted Value = '{extracted_value}', Correct Value = '{correct_value}'")
    return round(SequenceMatcher(None, extracted_value, correct_value).ratio() * 100, 2)

# Calculate attribute accuracy based on the type of field
...
def calculate_overall_accuracy(extracted_data, correct_data):
    ...
    if accuracies:  # Ensure accuracies list is not empty
        ...
        return 0.0  # Default value if accuracies list is empty


# Function to calculate and save accuracies to the file
def calculate_extraction_accuracy(file_path):
    ...
    # Calculate accuracies for each row
    ...
    print("Accuracy calculations added to the file.")

# Example usage:
csv_path = '/Users/janekkorczynski/Desktop/PDFExtraction/test_cv_extraction - extracted_information_test_cv.csv'
