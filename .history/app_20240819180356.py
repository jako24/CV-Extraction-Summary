...
def compare_and_merge_results(ocr_info, pypdf_info):
    merged_info = {}

    fields = ['name', 'contact_info', 'education', 'work_experience', 'skills', 'total_experience']

    def merge_field(ocr_value, pypdf_value, ocr_confidence, pypdf_confidence):
        if isinstance(ocr_value, str) and isinstance(pypdf_value, str):
            # For string fields, prefer the more detailed information
            if len(pypdf_value) > len(ocr_value) and ocr_value in pypdf_value:
                return pypdf_value, pypdf_confidence
            elif len(ocr_value) > len(pypdf_value) and pypdf_value in ocr_value:
            ...
            for key in set(ocr_value.keys()) | set(pypdf_value.keys()):
                if key in ocr_value and key in pypdf_value:
                    merged_dict[key], _ = merge_field(ocr_value[key], pypdf_value[key], ocr_confidence, pypdf_confidence)
            ...
            merged_list = []
            for ocr_item in ocr_value:
                if isinstance(ocr_item, dict):
                    matching_pypdf_item = next((item for item in pypdf_value if isinstance(item, dict) and
                        (item.get('institution') == ocr_item.get('institution') or
                         item.get('company_name') == ocr_item.get('company_name'))), None)
                    if matching_pypdf_item:
                        merged_item, _ = merge_field(ocr_item, matching_pypdf_item, ocr_confidence, pypdf_confidence)
                        merged_list.append(merged_item)
                    else:
                        merged_list.append(ocr_item)
                else:
                    merged_list.append(ocr_item)
            for pypdf_item in pypdf_value:
                if isinstance(pypdf_item, dict):
                    if not any(isinstance(item, dict) and
                               (item.get('institution') == pypdf_item.get('institution') or
                                item.get('company_name') == pypdf_item.get('company_name'))
                               for item in merged_list):
                        merged_list.append(pypdf_item)
                elif pypdf_item not in merged_list:
                    merged_list.append(pypdf_item)
            return merged_list, max(ocr_confidence, pypdf_confidence)
        else:
            return (ocr_value, ocr_confidence) if ocr_confidence > pypdf_confidence else (pypdf_value, pypdf_confidence)

    for field in fields:
        if field in ocr_info and field in pypdf_info:
            merged_value, merged_confidence = merge_field(
                ocr_info[field]['value'],
                pypdf_info[field]['value'],
                ocr_info[field]['confidence'],
                pypdf_info[field]['confidence']
            )
            merged_info[field] = {'value': merged_value, 'confidence': merged_confidence}
        elif field in ocr_info:
            merged_info[field] = ocr_info[field]
        elif field in pypdf_info:
            merged_info[field] = pypdf_info[field]

    return merged_info

def calculate_confidence(extracted_value, original_text):
    if not extracted_value:
        return 0

    if isinstance(extracted_value, str):
        if extracted_value.lower() in original_text.lower():
            return 1
        else:
            return 0.5  # Partial match or inference
    elif isinstance(extracted_value, list):
        return sum(calculate_confidence(item, original_text) for item in extracted_value) / len(extracted_value)
    elif isinstance(extracted_value, dict):
        confidences = [calculate_confidence(value, original_text) for value in extracted_value.values()]
        return sum(confidences) / len(confidences)
    else:
        return 0.5  # Default confidence for unknown types

def add_confidence_scores(information, original_text):
  return {key: {'value': value, 'confidence': calculate_confidence(value, original_text)}
...
def process_resume(uploaded_file):
    try:
        ...
        text_pypdf, information_pypdf, summary_pypdf = pypdf_loader(uploaded_file)

        if not information_pypdf:
            print("Failed to extract information using PyPDFLoader.")
            st.error("Failed to extract information using PyPDFLoader.")
            return None, None, None

        incomplete_fields = analyze_completeness(information_pypdf)
        print(f"Incomplete fields after PyPDFLoader: {incomplete_fields}")

        if incomplete_fields:
            print("Starting OCR extraction to fill incomplete fields...")
            text_ocr, information_ocr, summary_ocr = ocr(uploaded_file, show_boxes=False)
            if information_ocr:
                enhanced_information = enhance_with_ocr(information_pypdf, information_ocr, incomplete_fields)
                print("Enhanced information with OCR data")
            else:
                print("OCR extraction failed. Using PyPDFLoader information only.")
                st.warning("OCR extraction failed. Using PyPDFLoader information only.")
                enhanced_information = information_pypdf
        else:
            print("No incomplete fields. Using PyPDFLoader information only.")
            enhanced_information = information_pypdf
            text_ocr = ""

        final_summary = gpt3_generate_summary(enhanced_information)

        merged_text = f"PyPDFLoader Extracted Text:\n{text_pypdf}\n\nOCR Extracted Text:\n{text_ocr}"

        save_extracted_information(enhanced_information, method='Enhanced', file_name=uploaded_file.name)

        print("Resume processing completed successfully")
        return merged_text, enhanced_information, final_summary
    except Exception as e:
        print(f"An error occurred while processing the resume: {str(e)}")
