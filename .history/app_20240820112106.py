...
def merge_information(ocr_info, pypdf_info):
    def merge_lists(list1, list2):
        return list(set(item['value'] for item in list1 + list2))

    def merge_education(ocr_edu, pypdf_edu):
        merged_edu = []
        for edu in ocr_edu + pypdf_edu:
            degree = edu['degree']['value'].lower()
            grad_date = edu['graduation_date']['value']
            existing = next((x for x in merged_edu if x['degree']['value'].lower() == degree and x['graduation_date']['value'] == grad_date), None)
            if existing:
                for key in edu:
                    if edu[key]['confidence'] > existing[key]['confidence']:
    ...
    for key in all_keys:
        if key not in ocr_info:
            ...
            if isinstance(ocr_info[key], dict) and isinstance(pypdf_info[key], dict):
                if key == 'skills':
                    merged[key] = {
                        'technical_skills': merge_lists(ocr_info[key]['technical_skills'], pypdf_info[key]['technical_skills']),
                        'soft_skills': merge_lists(ocr_info[key]['soft_skills'], pypdf_info[key]['soft_skills'])
                    }
                else:
                    merged[key] = {
                        subkey: (ocr_info[key][subkey] if ocr_info[key][subkey]['confidence'] >= pypdf_info[key][subkey]['confidence']
                                 else pypdf_info[key][subkey])
                ...
                merged[key] = ocr_info[key] if ocr_info[key]['confidence'] >= pypdf_info[key]['confidence'] else pypdf_info[key]
