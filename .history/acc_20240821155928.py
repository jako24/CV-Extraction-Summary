...
def calculate_extraction_accuracy(file_path):
    ...
    def safe_json_loads(data):
        try:
            ...
            print(f"Successfully parsed JSON: {loaded_data}")
            return loaded_data
        except json.JSONDecodeError:
    ...
    # Calculate LoaderAccuracy and print debug info
    df['LoaderAccuracy'] = df.apply(lambda row: calculate_overall_accuracy(
        safe_json_loads(row['PDFLoaderExtraction'].lower() if isinstance(row['PDFLoaderExtraction'], str) else row['PDFLoaderExtraction']), 
        safe_json_loads(row['CorrectExtraction'].lower() if isinstance(row['CorrectExtraction'], str) else row['CorrectExtraction'])
    ), axis=1)
    
    print(df[['LoaderAccuracy']].head())  # Debug statement to print accuracy values
    
    # Similarly for other accuracy columns
    df['OCRAccuracy'] = df.apply(lambda row: calculate_overall_accuracy(
        safe_json_loads(row['OCRExtraction'].lower() if isinstance(row['OCRExtraction'], str) else row['OCRExtraction']), 
        safe_json_loads(row['CorrectExtraction'].lower() if isinstance(row['CorrectExtraction'], str) else row['CorrectExtraction'])
    ), axis=1)
    
    df['CombinedOCRAccuracy'] = df.apply(lambda row: calculate_overall_accuracy(
        safe_json_loads(row['CombinedOCRPDFExtraction'].lower() if isinstance(row['CombinedOCRPDFExtraction'], str) else row['CombinedOCRPDFExtraction']), 
        safe_json_loads(row['CorrectExtraction'].lower() if isinstance(row['CorrectExtraction'], str) else row['CorrectExtraction'])
    ), axis=1)

    df['EasyOCRAccuracy'] = df.apply(lambda row: calculate_overall_accuracy(
        safe_json_loads(row['EasyOCRExtraction'].lower() if isinstance(row['EasyOCRExtraction'], str) else row['EasyOCRExtraction']), 
        safe_json_loads(row['CorrectExtraction'].lower() if isinstance(row['CorrectExtraction'], str) else row['CorrectExtraction'])
    ), axis=1)
...
# Usage example
csv_path = '/Users/janekkorczynski/CVExtraction/TestAccuracyFailNoCommas.csv'
