...
def close_existing_tunnels():
    """
    Ensure all active ngrok tunnels are closed and the ngrok process is reset.
    """
    try:
        ...
        # Ensure ngrok process is killed
        ngrok.kill()
        print("All ngrok tunnels have been closed, and ngrok process has been killed.")
    except Exception as e:
        print(f"Error while closing tunnels: {e}")

def start_ngrok(port):
    """
    Start ngrok to tunnel the Streamlit app.
    """
    # Close any existing tunnels and reset ngrok
...
def stop_ngrok_on_exit():
    """
    Stop all ngrok tunnels when the app exits.
    """
    ngrok.kill()
    print("Ngrok process killed on exit.")

# Ensure ngrok is killed when the app exits
import atexit
atexit.register(stop_ngrok_on_exit)
