...
def compare_candidates(candidates_data, job_description):
    ...
    for candidate in candidates_data:
        ...
        job_experience_required = int(job_description.get('experience_required', 0))
        ...
        if experience_gap >= 0:
            # Bonus for meeting or exceeding the experience requirement
            score += 5 + experience_gap  # More points for having more experience than required
        else:
            # Deduct points for falling short of experience requirements
            score += max(0, 5 + experience_gap)  # Deduct points, but ensure score does not go negative
