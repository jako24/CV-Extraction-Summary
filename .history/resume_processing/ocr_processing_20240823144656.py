...
def pdf_to_img(pdf_file):
    # Save the uploaded PDF file to a temporary location for processing
    ...
    # Convert each page of the PDF to images at 500 DPI resolution
    pdf_pages = convert_from_path(temp_file_path, dpi=500)
    
    img_list = []
    for i, page in enumerate(pdf_pages, start=1):
        # Save each page as a JPEG image and add it to the list
...
def bounding_boxes(img_list, show_boxes):
    ...
    for curr_img in img_list:
        ...
        # Convert to grayscale for better accuracy in edge detection
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        
        # Apply Gaussian blur to smooth the image
        blur = cv2.GaussianBlur(gray, ksize=(5, 5), sigmaX=0)
        
        # Apply binary thresholding to isolate text regions
        _, thresh = cv2.threshold(blur, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        
        # Use dilation to enhance the contours of text regions
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (5, 5))
        dilate = cv2.dilate(thresh, kernel, iterations=1)
        
        # Find contours in the image, representing text regions
        contours, _ = cv2.findContours(dilate, mode=cv2.RETR_EXTERNAL, method=cv2.CHAIN_APPROX_SIMPLE)

        temp = []
        for contour in contours:
            x, y, w, h = cv2.boundingRect(contour)
            # Only consider larger contours (filtering out noise)
            if cv2.contourArea(contour) < 5000:
                continue
            temp.append([x, y, w, h])           # Store bounding box coordinates
        ...
        # Store the bounding boxes for this image            
        boxes[curr_img] = temp
...
def extract_text_ocr(pdf_file, file_name, show_boxes=False):
    ...
    # Convert the PDF to images for each page at 300 DPI resolution
    pdf_pages = convert_from_path(temp_file_path, dpi=300)
    
    # Determine if GPU is available for EasyOCR
...
def find_gaps(section_densities, section_width, threshold=0.1):
    # Identify gaps between sections based on density (to separate columns of text)
    gaps = []
    for i in range(1, len(section_densities)):
        # If the density of the section is below the threshold, mark it as a gap
...
def extract_text_dynamic_column(img_list):
    ...
    # Process each image to dynamically detect and extract columns of text
    for img_path in img_list:
        img = cv2.imread(img_path)
        if img is None:
        ...
        # Split the image into 20 sections for column detection
        num_sections = 20                                   
        section_width = width // num_sections
        section_densities = []

        # Calculate the density of each section for gap detection
        ...
        # Find gaps between sections to separate columns
        gaps = find_gaps(section_densities, section_width)

        if gaps:
            # If gaps are detected, split the image into left and right columns
            left_img = img[:, :gaps[0]]
            ...
            # If no gaps are detected, split the image in half and extract text from both halves
...
def ocr(uploaded_file, show_boxes):
    ...
    try:
        # Extract the file name for logging and identification
        ...
        # If requested, show bounding boxes and return early
        if show_boxes:
            bounding_boxes(img_list, show_boxes)
            return None, None, None

        # Perform dynamic column text extraction from the images
        text = extract_text_dynamic_column(img_list)
        logger.info(f"Extracted text length: {len(text)}")

        # Extract structured information from the text using GPT-3
        information = gpt3_extract_information(text)
        logger.info("Information extracted using GPT-3")

        # Generate a summary for the extracted information
        summary = gpt3_generate_summary(information)
        logger.info("Summary generated")

         # Save the extracted information to a CSV or file system
...
def process_resume(uploaded_file):
    try:
        ...
        # Step 4: Generate summary based on the job description
        ...
        try:
            ...
            if 'job_data' in st.session_state:
                ...
                final_summary = gpt3_generate_summary(merged_information, job_description)
            else:
                raise ValueError("Job description not available. Please scrape and process the job description first.")
