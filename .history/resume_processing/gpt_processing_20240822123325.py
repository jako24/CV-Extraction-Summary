...
def gpt3_generate_summary(information, job_description):
    ...
    prompt = f"""
    Based on the following candidate information and job description, generate a concise summary and a brief recommendation for a recruiter.
    ...
    1. Summarize the candidate's relevant experience and skills.
    2. Briefly evaluate how well the candidate fits the job requirements.
    3. Provide a simple recommendation on whether the candidate is a good fit for the role or suggest a more suitable role.
    ...
    response = openai.ChatCompletion.create(
        model="gpt-3.5-turbo",
        messages=[
            {"role": "system", "content": "You are an experienced recruiter tasked with providing a concise and actionable summary for a hiring decision."},
            {"role": "user", "content": prompt},
        ],
        max_tokens=600,  # Reduced token limit to encourage brevity
        n=1,
        stop=None,
        temperature=0.5,  # Keep temperature low for consistency
    )
