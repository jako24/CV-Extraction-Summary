...
def pypdf_loader(uploaded_file):
    if uploaded_file:
        ...
        # Ensure job description exists in session state before generating the summary
        if 'job_data' in st.session_state:
            job_description = st.session_state['job_data']
            summary = gpt3_generate_summary(information, job_description)
        else:
            st.error("Job description not available. Please scrape and process the job description first.")
