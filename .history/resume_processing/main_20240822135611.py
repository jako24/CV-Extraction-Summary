...
def main():
    ...
    if uploaded_file:
        ...
        with row1col1:
            if st.button(label='OCR + PDF'):
                with st.spinner("Processing resume with OCR and PyPDFLoader..."):
                    merged_text, enhanced_info, final_summary = process_resume(uploaded_file)

                    if merged_text:
                        st.session_state.flag_processed = True
                        st.session_state.candidates_data.append(enhanced_info)
                        st.success(f"Candidate data added: {enhanced_info['name']}")
                    else:
                        st.error("Resume processing failed.")

            if st.session_state.flag_processed:
                st.subheader("Combined Extracted Text")
                st.text_area("Combined Extracted Text", merged_text, height=300)
                
                st.subheader("Enhanced Information")
                st.json(enhanced_info)
        ...
        with row1col2:
            if st.session_state.flag_processed:
                ...
                final_summary = gpt3_generate_summary(enhanced_info, job_description)
                st.write(final_summary)

    # Display top candidates and compare after multiple uploads
    if st.button("Compare Candidates"):
        if st.session_state.candidates_data and 'job_data' in st.session_state:
            ...
            st.warning("Please upload resumes and process the job description before comparing candidates.")
