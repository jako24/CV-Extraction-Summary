...
def validate_merged_information(merged_info):
    ...
    if 'education' in validated_info:
        ...
        for edu in validated_info['education']:
            if all(edu[k]['value'] for k in ['degree', 'institution', 'graduation_date']):
    ...
    if 'work_experience' in validated_info:
        ...
        for work in validated_info['work_experience']:
            if all(work[k]['value'] for k in ['company_name', 'job_title', 'start_date']):
    ...
    if 'skills' in validated_info:
        validated_info['skills']['technical_skills'] = list({skill['value'] for skill in validated_info['skills'].get('technical_skills', [])})
        validated_info['skills']['soft_skills'] = list({skill['value'] for skill in validated_info['skills'].get('soft_skills', [])})
...
def merge_information(ocr_info, pypdf_info):
    ...
    def merge_education(ocr_edu, pypdf_edu):
        ...
        return sorted(merged_edu, key=lambda x: x['degree']['confidence'], reverse=True)
            ...
            if isinstance(ocr_info[key], dict) and isinstance(pypdf_info[key], dict):
                if key == 'skills':
                    ...
                    for subkey in set(ocr_info[key].keys()) | set(pypdf_info[key].keys()):
                        if subkey not in ocr_info[key]:
                            merged[key][subkey] = pypdf_info[key][subkey]
                        elif subkey not in pypdf_info[key]:
                            merged[key][subkey] = ocr_info[key][subkey]
                        else:
                            merged[key][subkey] = ocr_info[key][subkey] if ocr_info[key][subkey]['confidence'] >= pypdf_info[key][subkey]['confidence'] else pypdf_info[key][subkey]
