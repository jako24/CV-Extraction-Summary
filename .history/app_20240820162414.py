...
def save_extracted_information(information, method, file_name, csv_path = '/Users/janekkorczynski/CVExtraction/test_cv_extraction - extracted_information_test_cv copy.csv'):
    ...
    if os.path.exists(csv_path):
        ...
        if not 'OCRExtraction' in existing_data.columns:
            existing_data['CombinedOCRPDFExtraction'] = ''
        if not 'CombinedOCRPDFExtraction' in existing_data.columns:
            existing_data['EasyOCRExtraction'] = ''            
    ...
    # Check if the file name already exists in the data
    if file_name in existing_data['FileName'].values:
        # Update the relevant column based on the method
        if method == 'OCR':
            extraction_key = 'OCRExtraction'
        elif method == 'PDFLoader':
            extraction_key = 'PDFLoaderExtraction'
        elif method == 'Combined':
            extraction_key = 'CombinedOCRPDFExtraction'
        elif method == 'EasyOCR':
            extraction_key = 'EasyOCRExtraction'
        else:
