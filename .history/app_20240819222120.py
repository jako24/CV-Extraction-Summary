...
def merge_information(ocr_info, pypdf_info):
    ...
    def merge_simple_fields(field1, field2):
        if field1 is None:
            return field2
        if field2 is None:
            return field1
        ...
        if isinstance(field1, str) and isinstance(field2, str):
            return max(field1, field2, key=len)
        return field1  # Default to first field if types don't match
    ...
    def merge_lists(list1, list2, key):
        if key in ['education', 'work_experience']:
            ...
            for item in all_items:
                if not any(are_items_similar(item, existing) for existing in merged):
                    merged.append(item)
            return merged
        else:  # For skills
    ...
    def are_items_similar(item1, item2):
        if isinstance(item1, dict) and isinstance(item2, dict):
            return (normalize_string(item1.get('institution', '')) == normalize_string(item2.get('institution', '')) or
                    normalize_string(item1.get('company_name', '')) == normalize_string(item2.get('company_name', '')))
        return normalize_string(str(item1)) == normalize_string(str(item2))

    merged = {}

    # Merge simple fields
    for key in ['name', 'contact_info', 'total_experience']:
        if key in ocr_info and key in pypdf_info:
            if isinstance(ocr_info[key], dict) and isinstance(pypdf_info[key], dict):
                merged[key] = {k: merge_simple_fields(ocr_info[key].get(k), pypdf_info[key].get(k)) 
                               for k in set(ocr_info[key].keys()) | set(pypdf_info[key].keys())}
            else:
                merged[key] = merge_simple_fields(ocr_info[key], pypdf_info[key])
        elif key in ocr_info:
            merged[key] = ocr_info[key]
        elif key in pypdf_info:
            merged[key] = pypdf_info[key]

    # Merge lists (education, work_experience, skills)
    for key in ['education', 'work_experience', 'skills']:
        if key in ocr_info and key in pypdf_info:
            merged[key] = merge_lists(ocr_info[key], pypdf_info[key], key)
        elif key in ocr_info:
            merged[key] = ocr_info[key]
        elif key in pypdf_info:
            merged[key] = pypdf_info[key]
