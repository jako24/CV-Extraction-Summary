...
def merge_information(ocr_info, pypdf_info):
    def are_lists_similar(list1, list2, threshold=0.7):
        if not list1 or not list2:
            return False
        common_items = set(list1) & set(list2)
        similarity = len(common_items) / max(len(list1), len(list2))
        return similarity >= threshold

    def merge_lists(list1, list2):
        merged = list(set(list1 + list2))  # Remove duplicates
        return merged

    def merge_dicts(dict1, dict2):
        merged = dict1.copy()
        for key, value in dict2.items():
            if key == 'contact_info' and 'email' in value:
                merged['contact_info']['email'] = value['email']
            elif key == 'skills':
                merged['skills']['technical_skills'] = merge_lists(
                    merged['skills'].get('technical_skills', []),
                    value.get('technical_skills', [])
                )
                merged['skills']['soft_skills'] = merge_lists(
                    merged['skills'].get('soft_skills', []),
                    value.get('soft_skills', [])
                )
                ...
                if not are_lists_similar(merged[key], value):
                    merged[key] = merge_lists(merged[key], value)
            elif key != 'contact_info' and key != 'skills':  # Prefer OCR for other fields
                continue
