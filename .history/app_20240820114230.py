...
def merge_information(ocr_info, pypdf_info):
    def merge_lists(list1, list2):
        if all(isinstance(item, dict) for item in list1 + list2):
            # For lists of dictionaries (like education or work experience)
            merged = list1 + list2
            unique_items = []
            seen = set()
            for item in merged:
            ...
            # For lists of strings (like skills)
            return list(set(list1 + list2))

    def merge_dict_values(v1, v2):
        if isinstance(v1, dict) and isinstance(v2, dict):
            return {k: merge_dict_values(v1.get(k), v2.get(k)) for k in set(v1) | set(v2)}
