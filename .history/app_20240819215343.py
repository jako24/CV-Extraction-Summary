...
def merge_information(ocr_info, pypdf_info):
    ...
    def are_jobs_similar(job1, job2):
        company_similarity = normalize_string(job1['company_name']) == normalize_string(job2['company_name'])
        title_similarity = normalize_string(job1['job_title']) in normalize_string(job2['job_title']) or normalize_string(job2['job_title']) in normalize_string(job1['job_title'])
        date_similarity = normalize_date(job1['start_date']) == normalize_date(job2['start_date']) or normalize_date(job1['end_date']) == normalize_date(job2['end_date'])
        return company_similarity and (title_similarity or date_similarity)

    def merge_work_experience(exp1, exp2):
        merged = []
        for job1 in exp1:
            similar_job = next((job2 for job2 in exp2 if are_jobs_similar(job1, job2)), None)
            if similar_job:
                merged_job = {
                    'company_name': job1['company_name'],
                    'job_title': max(job1['job_title'], similar_job['job_title'], key=len),
                    'start_date': min(job1['start_date'], similar_job['start_date'], key=lambda d: parse(d, fuzzy=True)),
                    'end_date': max(job1['end_date'], similar_job['end_date'], key=lambda d: parse(d, fuzzy=True) if d.lower() != 'now' and d.lower() != 'present' else datetime.max)
                }
        ...
        for job2 in exp2:
            if not any(are_jobs_similar(job2, job) for job in merged):
                merged.append(job2)
        
        return sorted(merged, key=lambda x: parse(x['start_date'], fuzzy=True), reverse=True)

    def merge_lists(list1, list2, key):
        if key == 'work_experience':
            return merge_work_experience(list1, list2)
        else:
            # Use the previous merge_lists logic for other list types
            merged = list1.copy()
            for item2 in list2:
                if isinstance(item2, dict):
                    if not any(are_dicts_similar(item2, item1) for item1 in merged if isinstance(item1, dict)):
                        merged.append(item2)
                elif item2 not in merged:
                    merged.append(item2)
            return merged

    def are_dicts_similar(dict1, dict2, threshold=0.7):
        common_keys = set(dict1.keys()) & set(dict2.keys())
        if not common_keys:
            return False
        
        similarity = sum(normalize_string(str(dict1[key])) == normalize_string(str(dict2[key])) for key in common_keys) / len(common_keys)
    ...
    def merge_dicts(dict1, dict2):
        ...
        for key, value in dict2.items():
            if key not in merged:
                ...
                merged[key] = merge_lists(merged[key], value, key)
            elif value and not merged[key]:  # Prefer non-empty values
                merged[key] = value
        return merged

    return merge_dicts(ocr_info, pypdf_info)
