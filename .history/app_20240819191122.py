...
def compare_and_merge_results(ocr_info, pypdf_info, original_text):
    ...
    def calculate_confidence(value, text):
        if isinstance(value, str):
            ...
            return sum(calculate_confidence(item, text) for item in value) / len(value) if value else 0
        elif isinstance(value, dict):
            return sum(calculate_confidence(v, text) for v in value.values()) / len(value) if value else 0
        else:
            return 0.5
    ...
    def merge_field(ocr_value, pypdf_value):
        ...
        if isinstance(ocr_value, str) and isinstance(pypdf_value, str):
            if ocr_value.lower() != pypdf_value.lower():
                combined = f"{ocr_value} {pypdf_value}".strip()
                return combined, max(ocr_confidence, pypdf_confidence)
            return (ocr_value, ocr_confidence) if len(ocr_value) >= len(pypdf_value) else (pypdf_value, pypdf_confidence)
            ...
            # Combine lists, removing duplicates but keeping all unique information
            merged_list = []
            for item in ocr_value + pypdf_value:
                if item not in merged_list:
                    if isinstance(item, dict):
                        if not any(all(k in d and d[k] == item[k] for k in item) for d in merged_list):
...
def process_resume(uploaded_file):
    try:
        ...
        if not information_pypdf and not information_ocr:
            print("Failed to extract information using both PyPDFLoader and OCR.")
            st.error("Failed to extract information using both PyPDFLoader and OCR.")
            return None, None, None

        merged_text = f"PyPDFLoader Extracted Text:\n{text_pypdf}\n\nOCR Extracted Text:\n{text_ocr}"
        enhanced_information = compare_and_merge_results(information_ocr, information_pypdf, merged_text)

        # Additional step to ensure we're not losing any information
        for field in enhanced_information:
            if field in information_ocr and field in information_pypdf:
                ocr_value = information_ocr[field]
                pypdf_value = information_pypdf[field]
                if isinstance(ocr_value, str) and isinstance(pypdf_value, str):
                    enhanced_information[field]['value'] = f"{ocr_value} {pypdf_value}".strip()
                elif isinstance(ocr_value, list) and isinstance(pypdf_value, list):
                    merged_list = []
                    for item in ocr_value + pypdf_value:
                        if isinstance(item, dict):
                            ...
                            merged_list.append(item)
                    enhanced_information[field]['value'] = merged_list
