...
def validate_merged_information(merged_info):
    ...
    if 'education' in validated_info:
        ...
        for edu in validated_info['education']:
            if all(edu.get(k) for k in ['degree', 'institution', 'graduation_date']):
    ...
    if 'work_experience' in validated_info:
        ...
        for work in validated_info['work_experience']:
            if all(work.get(k) for k in ['company_name', 'job_title', 'start_date']):
    ...
    if 'skills' in validated_info:
        validated_info['skills']['technical_skills'] = list(set(validated_info['skills'].get('technical_skills', [])))
        validated_info['skills']['soft_skills'] = list(set(validated_info['skills'].get('soft_skills', [])))

    return validated_info

def merge_information(ocr_info, pypdf_info):
    def merge_lists(list1, list2):
        return list(set(list1 + list2))  # Remove duplicates

    def merge_education(ocr_edu, pypdf_edu):
        if not ocr_edu:
            return pypdf_edu
        
        merged_edu = ocr_edu.copy()
        
        for pypdf_entry in pypdf_edu:
            matching_entry = next((entry for entry in merged_edu 
                                   if entry['degree'].lower() == pypdf_entry['degree'].lower() 
                                   and entry['graduation_date'] == pypdf_entry['graduation_date']), None)
            if matching_entry:
                # Fill in missing information from PyPDF, but don't overwrite existing OCR info
                for key, value in pypdf_entry.items():
                    if not matching_entry.get(key):
                        matching_entry[key] = value
            else:
                # Add PyPDF entry only if it's not already represented in OCR data
                if not any(entry['degree'].lower() == pypdf_entry['degree'].lower() for entry in merged_edu):
                    merged_edu.append(pypdf_entry)
        
        return merged_edu

    merged = ocr_info.copy()  # Start with OCR info as the base

    # Ensure email from PyPDF is used if available and OCR email is missing
    if 'contact_info' in merged and 'contact_info' in pypdf_info:
        if not merged['contact_info'].get('email') and pypdf_info['contact_info'].get('email'):
            merged['contact_info']['email'] = pypdf_info['contact_info']['email']

    # Merge skills
    if 'skills' in merged and 'skills' in pypdf_info:
        merged['skills']['technical_skills'] = merge_lists(
            merged['skills'].get('technical_skills', []),
            pypdf_info['skills'].get('technical_skills', [])
        )
        merged['skills']['soft_skills'] = merge_lists(
            merged['skills'].get('soft_skills', []),
            pypdf_info['skills'].get('soft_skills', [])
        )

    # Merge education
    if 'education' in merged or 'education' in pypdf_info:
        merged['education'] = merge_education(merged.get('education', []), pypdf_info.get('education', []))

    # For other fields, use PyPDF info only if OCR info is missing
    for key in pypdf_info:
        if key not in merged or not merged[key]:
            merged[key] = pypdf_info[key]
        elif isinstance(merged[key], dict) and isinstance(pypdf_info[key], dict):
            for sub_key, sub_value in pypdf_info[key].items():
