...
def merge_information(ocr_info, pypdf_info):
    def merge_skills(skills1, skills2):
        merged_skills = {
            "technical_skills": list(set(skills1.get("technical_skills", []) + skills2.get("technical_skills", []))),
            "soft_skills": list(set(skills1.get("soft_skills", []) + skills2.get("soft_skills", [])))
        }
        return merged_skills

    merged = ocr_info.copy()  # Start with OCR info as the base

    # Only merge contact info from pypdf_info
    ...
    # Merge skills
    if 'skills' in pypdf_info:
        merged['skills'] = merge_skills(merged.get('skills', {}), pypdf_info['skills'])
