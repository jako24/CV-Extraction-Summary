...
def merge_information(ocr_info, pypdf_info):
    ...
    for key, value in pypdf_info.items():
        if key not in merged_info or not merged_info[key]:
            merged_info[key] = value
        elif isinstance(value, dict) and isinstance(merged_info[key], dict):
            # Recursively merge nested dictionaries
            merged_info[key] = merge_information(merged_info[key], value)
        elif isinstance(value, list) and isinstance(merged_info[key], list):
            # Merge lists without converting to set
            merged_info[key] = merge_lists(merged_info[key], value)
        # If types don't match, prefer OCR info (which is already in merged_info)
