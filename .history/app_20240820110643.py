...
def merge_information(ocr_info, pypdf_info):
    ...
    def merge_education(ocr_edu, pypdf_edu):
        merged_edu = []
        for ocr_entry in ocr_edu:
            matching_entry = next((entry for entry in pypdf_edu if entry['degree'] == ocr_entry['degree'] and entry['graduation_date'] == ocr_entry['graduation_date']), None)
            if matching_entry:
                merged_entry = ocr_entry.copy()
                for key, value in matching_entry.items():
                    if not merged_entry.get(key):
                        merged_entry[key] = value
                merged_edu.append(merged_entry)
            else:
                merged_edu.append(ocr_entry)
        
        # # Add any PyPDF entries that weren't matched
        # for pypdf_entry in pypdf_edu:
        #     if not any(entry['degree'] == pypdf_entry['degree'] and entry['graduation_date'] == pypdf_entry['graduation_date'] for entry in merged_edu):
    ...
    # Ensure email from PyPDF is used if available
    if 'contact_info' in pypdf_info and 'email' in pypdf_info['contact_info']:
    ...
    if 'skills' in pypdf_info:
        if 'skills' not in merged:
            merged['skills'] = {'technical_skills': [], 'soft_skills': []}
    ...
    # Merge education
    if 'education' in ocr_info and 'education' in pypdf_info:
        merged['education'] = merge_education(ocr_info['education'], pypdf_info['education'])
    elif 'education' in ocr_info:
        merged['education'] = ocr_info['education']
    elif 'education' in pypdf_info:
        merged['education'] = pypdf_info['education']

    # For other fields, prefer OCR info
    for key, value in merged.items():
        if key not in ['contact_info', 'skills', 'education']:
            continue
        
        if isinstance(value, dict):
            for sub_key, sub_value in value.items():
                if not sub_value and key in pypdf_info and sub_key in pypdf_info[key]:
                    merged[key][sub_key] = pypdf_info[key][sub_key]
        elif isinstance(value, list) and not value:
            if key in pypdf_info:
...
def process_resume(uploaded_file):
    try:
        ...
        try:
            ...
            if not merged_information:
                raise ValueError("Information merging failed")
            logger.info("Information merging completed")
