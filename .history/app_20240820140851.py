...
def merge_information(ocr_info, pypdf_info):
    def merge_skills(ocr_skills, pypdf_skills):
        # Merge technical and soft skills from both sources
        merged_skills = {
    ...
    # Merge contact info from PyPDFLoader if present
    if 'contact_info' in pypdf_info:
        merged['contact_info'] = merged.get('contact_info', {})
        merged['contact_info']['email'] = pypdf_info['contact_info'].get('email') or merged['contact_info'].get('email', '')
        merged['contact_info']['phone'] = pypdf_info['contact_info'].get('phone') or merged['contact_info'].get('phone', '')

    # Merge education, if missing from OCR but present in PyPDFLoader
    if 'education' in pypdf_info and not merged.get('education'):
        merged['education'] = pypdf_info['education']

    # Merge work experience, if missing from OCR but present in PyPDFLoader
    if 'work_experience' in pypdf_info and not merged.get('work_experience'):
...
def process_resume(uploaded_file):
    try:
        ...
        # Step 1: OCR extraction (Priority given to OCR)
        logger.info("Performing OCR extraction...")
        try:
            # text_ocr = extract_text_ocr(uploaded_file)
            text_ocr = ocr(uploaded_file, show_boxes=False)
            if not text_ocr:
        ...
        # Step 2: PyPDFLoader extraction for contact info only
        logger.info("Performing PyPDFLoader extraction for contact info...")
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as temp_file:
                temp_file.write(uploaded_file.getbuffer())
                temp_file_path = temp_file.name
            ...
            # Only extract the contact info like email and phone from PyPDFLoader
            ...
            logger.info("PyPDFLoader contact info extraction completed")
        except Exception as pypdf_error:
            logger.error(f"Error in PyPDFLoader extraction: {str(pypdf_error)}")
            raise

        # Step 3: Merge OCR information with PyPDFLoader's contact info
        logger.info("Merging contact info from PyPDFLoader into OCR information...")
        try:
            # Copy OCR information as the base
            merged_information = information_ocr.copy()

            # Merge contact info from PyPDFLoader
            if 'contact_info' in information_pypdf:
                merged_information['contact_info'] = merged_information.get('contact_info', {})
                if not merged_information['contact_info'].get('email'):
                    merged_information['contact_info']['email'] = information_pypdf['contact_info'].get('email', '')
                if not merged_information['contact_info'].get('phone'):
                    merged_information['contact_info']['phone'] = information_pypdf['contact_info'].get('phone', '')

            # Log the merged information for debugging
            logger.info(f"Merged Information: {json.dumps(merged_information, indent=2)}")
            logger.info("Contact info merging completed")
        except Exception as merge_error:
            logger.error(f"Error in merging contact info: {str(merge_error)}")
            raise
        ...
        # Combine OCR and PyPDF texts for logging or further analysis
        # merged_text = f"OCR Extracted Text:\n{text_ocr}\n\nPyPDFLoader Extracted Contact Info:\n{text_pypdf}"
        merged_text = f"OCR Extracted Text:\n{text_ocr}\n"
